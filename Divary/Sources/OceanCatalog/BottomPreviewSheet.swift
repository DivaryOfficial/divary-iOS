...
struct BottomPreviewSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("\(sampleSeaCreatureDetail.name) (\(sampleSeaCreatureDetail.appearance.pattern))")
                .font(.omyu.regular(size: 24))
                .bold()
            Text(sampleSeaCreatureDetail.type)
                .foregroundStyle(Color(.grayscaleG600))
                .font(.NanumSquareNeo.NanumSquareNeoRegular(size: 14))

            HStack(spacing: 12) {
                AsyncImage(url: sampleSeaCreatureDetail.imageUrls.first) { phase in
                ...
                VStack(alignment: .leading, spacing: 6) {
                    LabelledText(title: "크기", value: sampleSeaCreatureDetail.size)
                    LabelledText(title: "출몰시기", value: sampleSeaCreatureDetail.appearPeriod)
                    LabelledText(title: "서식", value: sampleSeaCreatureDetail.place)
                }
                .font(.subheadline)
            }
